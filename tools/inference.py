...
def draw_boxes(img, bboxs, cls_label):	
	...
	img_draw = np.array(img.resize((max_orig_size, max_orig_size), Image.Resampling.LANCZOS).copy())

	for i in range(len(bboxs)):
		x1, y1, x2, y2 = bboxs[i]
		color = bbox_colors[int(np.where(unique_labels == cls_label[i])[0])]
		img_draw = cv2.rectangle(img_draw, (x1, y1), (x2, y2), color, 2)
		img_draw = cv2.putText(img_draw, MASK_RUS[cls_label[i]], (x1, y1 - 2), cv2.FONT_HERSHEY_COMPLEX, 
