...
def train(train_loader, val_loader, model, optimizer, opt, cfg, scheduler=None):
    nb = len(train_loader)
    n_burn = max(3 * nb, 500)
    ...
    for epoch in tqdm(range(0, opt.epoches)):
        ...
        for i, (_, imgs, targets) in enumerate(train_loader):
            ...
            if ni <= n_burn:
                ...
                accumulate = max(1, np.interp(ni, xi, [1, 
                    cfg.accumulate_batch / opt.batch_size]).round())
                for j, p_model in enumerate(optimizer.param_groups):
                    p_model['lr'] = np.interp(ni, xi, [0.1 if j == 2 else 0.0,
                        p_model['initial_lr'] * lf(epoch)])
                    p_model['weight_decay'] = np.interp(ni, xi,
                        [0.0, opt.lr if j == 1 else 0.0])
...
if __name__ == '__main__':
    ...
    # scheduler CosLR
    lf = lambda x: (((1 + math.cos(x * math.pi / opt.epoches)) / 2) ** 1.0)\
        * 0.95 + 0.05
    ...
    TrainLoader = DataLoader(trainset, batch_size=opt.batch_size, collate_fn=trainset.collate_fn,
        sampler=train_sampler, num_workers=opt.n_work)
    ValidationLoader = DataLoader(testset, batch_size=opt.batch_size, collate_fn=trainset.collate_fn,
        sampler=validation_sampler, num_workers=opt.n_work)
