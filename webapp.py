...
@st.cache_data
def load_model():
    cfg = Params(f'projects/dior.yml')
...
def main():
    ...
    with st.expander("Подробнее о приложении"):
        st.write('Привет!👋 Это сервис для обнаружения объектов на спутниковых снимках🌍🛰️.')
        st.write("Модель обучена на наборе данных [DIOR](https://arxiv.org/abs/1909.00133). Здесь содержится 20 различных классов,\
                    которые могут быть на спутниковых снимках✈🚤🚞🏟🏭🌉.")
        st.image('./imgs/classes.png')
        st.write("Отправь мне картинку/скриншот со спутников🌍🛰️, программа попытается найти объекты на нем🔍.\
                    Можно воспользоваться [Google Earth](https://earth.google.com/web/), [Google Map](https://www.google.com/maps) или [Yandex Map](https://yandex.ru/maps).\nРекомендуется использовать Google Earth.")
        st.write("Здесь используется одна из моделей Object Detection [YOLOv3](https://arxiv.org/abs/1804.02767).")
        st.write("Репозиторий [GitHub](https://github.com/Vlad15lav/ors-detection) обученной модели.")
    ...
    if uploaded_file is not None:
        ...
        with col1:
            st.markdown(f"<div style='text-align: center;'>{'Оригинальное изображение'}</div>", unsafe_allow_html=True)
            # st.write('Оригинальное изображение')
            st.image(uploaded_file)
            image = Image.open(uploaded_file)

        bbox, cls_label, text_statistic = model_inference(model, image, conf_tresh=conf)
        img_draw = draw_boxes(image, bbox, cls_label)

        with col2:
            # st.write('Предсказание модели')
            st.markdown(f"<div style='text-align: center;'>{'Предсказание модели'}</div>", unsafe_allow_html=True)
            st.image(img_draw)
    ...
    if select_region:
        ...
        if uploaded_file is None:
            st.write("Не удалось получить спутниковое изображени! Ошибка в координатах")
        else:
            col1, col2 = st.columns(2)
            with col1:
                st.markdown(f"<div style='text-align: center;'>{'Оригинальное изображение'}</div>", unsafe_allow_html=True)
            ...
            bbox, cls_label, text_statistic = model_inference(model, uploaded_file, conf_tresh=conf)
            ...
            folium.Marker(
                [latitude, longitude], popup="Текущие координаты", tooltip=f"Текущие координаты: {longitude} {latitude}"
            ).add_to(m)
