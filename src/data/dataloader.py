...
from tools.box import xyxy2xywh
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, path, opt, cfg, transform=None):
        ...
        self.label_files = [
            x.replace("images", "labels").replace(os.path.splitext(x)[-1],
                                                  ".txt")
        ...
        for i, file in enumerate(self.label_files):
            try:
                with open(file, "r") as f:
                    ...
                    org_label[:, 1] = orig_w * (label[:, 1] - label[:, 3] / 2)
                    org_label[:, 2] = orig_h * (label[:, 2] - label[:, 4] / 2)
                    org_label[:, 3] = orig_w * (label[:, 1] + label[:, 3] / 2)
                    org_label[:, 4] = orig_h * (label[:, 2] + label[:, 4] / 2)
                    org_label = np.round(org_label)

            except Exception:
                continue

            # not empty targets
            if org_label.shape[0]:
                self.labels[i] = org_label
    ...
    def load_annotations(self, index):
        boxes = np.array(self.labels[index][:, 1:],
                         np.float32).reshape((-1, 4))
        label = np.array(self.labels[index][:, 0],
                         np.int64).reshape((-1,))
