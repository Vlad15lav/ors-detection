...
class MySQL:
    ...
    def create_connection(
        ...
        try:
            self.connection = mysql.connector.connect(
                ...
                passwd=user_password, database=databases
            )
        except Error:
            flag = False
    ...
    def select(self, query: str):
        try:
            ...
            if not self.create_connection(
                ...
                self.user_password, self.database
            ):
    ...
    def add_request(self, user, id_cls, counts):
        id_user = self.select("SELECT id_user FROM users" +
        ...
        if len(id_user) == 0:
            ...
            id_user = self.select(
                f"SELECT id_user FROM users WHERE user_name = '{user}'"
            )
        id_user = id_user[0][0]
        ...
        for i in range(len(id_cls)):
            self.insert(
                "INSERT INTO requests (id_user, id_cls, count, date) " +
                "VALUES (%s, %s, %s, %s)",
    ...
    def get_popular_all_class(self):
        ...
        if len(result):
            text = "Статистика по найденным объектом среди всех" \
                " пользователей:\n\n"
    ...
    def get_popular_all_last(self):
        ...
        result = self.select(qr_text)
        if len(result):
            text = "Статистика по найденным объектам среди" \
                " всех пользователей за сутки:\n\n"
            for row in result:
                text += f"{row[0]} - {row[1]}\n"
            return text.rstrip("\n")
        else:
    ...
    def get_count_user_last(self, user_name):
        qr_text = "SELECT c.class_name, SUM(r.count) AS sum_count \
            ...
            JOIN users AS u USING(id_user) \
            WHERE r.date > NOW() - INTERVAL 1 DAY " \
            f"AND u.user_name = '{user_name}' \
            GROUP BY c.class_name \
            ORDER BY sum_count DESC"

        result = self.select(qr_text)
        if len(result):
            text = "Статистика по найденным объектам " \
                f"{user_name} за сутки:\n\n"
            for row in result:
                text += f"{row[0]} - {row[1]}\n"
            return text.rstrip("\n")
        else:
...
INSERT INTO classes (class_name) VALUES ('airplane'), ('airport'),
('baseballfield'), ('basketballcourt'), ('bridge'),
('chimney'), ('dam'), ('Expressway-Service-area'), ('Expressway-toll-station'),
('golffield'), ('groundtrackfield'), ('harbor'), ('ship'), ('stadium'),
('storagetank'), ('tenniscourt'), ('trainstation'),
('vehicle'), ('windmill'), ('overpass');
