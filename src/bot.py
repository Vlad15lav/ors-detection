...
TEXT_HELLO = "Привет!👋 Это бот для обнаружения объектов " \
    "на спутниковых снимках🌍🛰️"
TEXT_CLASSES = "Модель обучена на наборе данных [DIOR]" \
    "(https://arxiv.org/abs/1909.00133). Здесь содержится 20 различных " \
    "классов, которые могут быть на спутниковых снимках✈🚤🚞🏟🏭🌉"
TEXT_FEATURES = "Отправь мне картинку/скриншот со спутников🌍🛰️, " \
    "программа попытается найти объекты на нем🔍 Можно воспользоваться " \
    "[Google Earth](https://earth.google.com/web/), [Google Map]" \
    "(https://www.google.com/maps) или [Yandex Map](https://yandex.ru/maps)" \
    "\n\nРекомендуется использовать Google Earth"
TEXT_MODEL = "Здесь используется одна из моделей Object Detection [YOLOv3]" \
    "(https://arxiv.org/abs/1804.02767)"
TEXT_EXAMPLE = "Демонстрация работы на некоторых снимках👁️ Репозиторий " \
    "[GitHub](https://github.com/Vlad15lav/ors-detection) обученной модели"
TEXT_MAP = "Отправьте [координаты](https://www.latlong.net/) долготы, " \
...
# База данных
database = MySQL()
if not database.create_connection(DATABASE_ADDRESS, DATABASE_USER,
                                  DATABASE_PASW, DATABASE_NAME):
    raise "Ошибка в подключение MySQL!"

# Детектор YoloV3
model = YoloV3()
model.load_state_dict(
    torch.load("weights/dior_weights.pth", map_location=torch.device("cpu"))
)
...
def handle_message(message):
    ...
    markup.add(btn_classes, btn_model, btn_example,
               btn_map, btn_question, btn_stats)
    ...
    if message.text == "Объекты📰":
        ...
        # Информаци о YoloV3
        bot.send_message(
            message.chat.id, text=TEXT_MODEL,
        ...
        if coord is not None:
            ...
            if imageFile is not None:
                # Поиск объектов
                model_result, text_statistic, user_stats = model_inference(
                    model, imageFile
                )
                if len(user_stats) > 0:
...
def handle_photo(message):
    ...
    # Поиск объектов
    model_result, text_statistic, user_stats = model_inference(model,
                                                               imageFile)
    if len(user_stats) > 0:
        database.add_request(
            message.from_user.username, user_stats.keys(), user_stats.values()
        )

    bot.send_photo(
        message.chat.id, model_result,
        caption=text_statistic, reply_markup=markup
    )
...
def handle_docs(message):
    ...
    try:
        ...
        if file_type in PICTURE_TYPES:
            ...
            # Поиск объектов
            model_result, text_statistic, user_stats = \
                model_inference(model, imageFile)
            if len(user_stats) > 0:
                database.add_request(
                    message.from_user.username,
                    user_stats.keys(),
